...
public class Main{
    public static void main(String[] args) throws IOException {
        ...
        String line;
        Integer businessAcc;
        String fileName = "Help\\information.csv";
        BigDecimal funds;
        ...
        for (File file : folder.listFiles()) {
            ...
            if (fileName.matches("^Help\\\\transaction[0-9]+\\.csv$")) {
                try {
                    BufferedReader r = new BufferedReader(new FileReader(fileName));
                    lineNum = 1;
                    r.readLine();
                        ...
                        if (companies.containsKey(businessAcc) && companies.get(businessAcc).getName().equals(content[0])) {
                            ...
                            if (companies.containsKey(businessAcc) && companies.get(businessAcc).getName().equals(content[2])) {
                                ...
                                if (remitter.debitFunds(funds)) {
                                    ...
                                    System.out.println("" + lineNum + ". Осуществлён перевод"
                                            + ". Отправитель: " + content[0] + ". Получатель: " + content[2] + ". Сумма: " + content[4]);
                                }else
                                    System.out.println( "" + lineNum + ". Нехватка денег");
                            }
                        }
        ...
        fileName = "Help\\result.csv";
        BufferedWriter w = new BufferedWriter(new FileWriter(fileName));
                w.write("");
                w.append("Имя компании;Рассчётный счёт;Ссума на ссчёте\n");
        System.out.println("\nResult");
